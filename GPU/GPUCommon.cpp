...
u32 GPUCommon::Break(int mode)
{
	...
	if(currentDisplayList->state == PSP_GE_DL_STATE_NONE ||
	   currentDisplayList->state == PSP_GE_DL_STATE_COMPLETED)
	    if(sceKernelGetCompiledSdkVersion() >= 0x02000000)
			return 0x80000004;
		else
			return -1;
	...
	case GE_CMD_RET:
		{
			if (currentList()->stackptr == 0) {
				...
				if (!Memory::IsValidAddress(currentList()->pc)) {
					ERROR_LOG(G3D, "Invalid DL PC %08x on return", pc);
				}
			}
		}
		break;
...
void GPUCommon::DoState(PointerWrap &p) {
	/*
	int currentID = currentDisplayList == NULL ? 0 : currentDisplayList->id;
	p.Do(currentID);
	if (currentID == 0) {
		currentDisplayList = 0;
	} else {
		for (auto it = dlQueue.begin(), end = dlQueue.end(); it != end; ++it) {
			if (it->id == currentID) {
				currentDisplayList = &*it;
				break;
			}
		}
	}*/
